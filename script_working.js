...
class SchoolManagementSystem {
    ...
    setupEventListeners() {
        ...
        document.getElementById('attendance-class-filter').addEventListener('change', () => this.renderAttendance());
        document.getElementById('attendance-search').addEventListener('input', () => this.renderAttendance());
        document.getElementById('attendance-date').addEventListener('change', () => this.renderAttendance());
        document.getElementById('bus-route-filter').addEventListener('change', () => this.renderBusSubscriptions());
        document.getElementById('bus-class-filter').addEventListener('change', () => this.renderBusSubscriptions());
        document.getElementById('fees-class-filter').addEventListener('change', () => this.renderFeePayments());
        document.getElementById('fees-month').addEventListener('change', () => this.renderFeePayments());
        document.getElementById('fee-status-filter').addEventListener('change', () => this.renderStudents());
...
saveRoute() {
    ...
    renderFeePayments() {
        const monthFilter = document.getElementById('fees-month').value;
        const classFilter = document.getElementById('fees-class-filter').value;
        
        let filteredPayments = this.feePayments;
        
        if (monthFilter) {
            filteredPayments = filteredPayments.filter(f => f.month === monthFilter);
        }
        
        if (classFilter) {
            const studentsInClass = this.students.filter(s => s.class === classFilter).map(s => s.id);
            filteredPayments = filteredPayments.filter(f => studentsInClass.includes(f.studentId));
        }
    ...
    updateStatistics() {
        ...
        const currentMonth = new Date().getMonth() + 1;
        const currentYear = new Date().getFullYear();
        const monthlyFees = this.feePayments.filter(f => 
            f.month === currentMonth && f.year === currentYear && f.status === 'paid'
        );
        const totalCollected = monthlyFees.reduce((sum, fee) => sum + fee.total, 0);
        document.getElementById('fee-collection').textContent = `$${totalCollected}`;
    ...
    updateFeeProgress() {
        ...
        const collectedFees = this.feePayments
            .filter(f => f.month === currentMonth && f.year === currentYear && f.status === 'paid')
            .reduce((sum, fee) => sum + fee.total, 0);

        // Calculate percentage
        const percentage = expectedFees > 0 ? Math.round((collectedFees / expectedFees) * 100) : 0;

        // Update UI
        document.getElementById('fees-collected-text').textContent = `$${collectedFees} collected`;
        document.getElementById('fees-total-text').textContent = `$${expectedFees} total`;
